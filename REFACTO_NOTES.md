...
| **4** Exceptionnels | re-skin de la feature existante (déjà 100% réelle) : KPI strip 4 cartes (dont **Part des dépenses** dérivée du `regularMonthlyAverage`), toolbar year-**chips** + **« Toutes les années »** (fetch year-optional) + Ajouter, groupes par mois (cartes `.exc-*`), modale reskin. CRUD/filtres/stats préservés. Reviewé (workflow adversarial, 1 finding hydration traité). tsc+eslint clean, **à valider en loggé** | _non commité_ |
...
- **Phase 5** — Dashboard mensuel (redesign `/overview` : insights [mock], prévision, hero budget restant + jauge, plafond hebdo, répartition catégories, dépenses fixes, sparkline). Renommer les routes proprement ici si voulu.
