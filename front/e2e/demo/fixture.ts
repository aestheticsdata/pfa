 ...
 *
 * PFA cannot fail that way today: its three faces come through
 * `next/font/google`, which self-hosts them at build time, so nothing the
 * document asks for is third-party — the `fonts.googleapis.com` entry in its
 * CSP is a leftover, not a fetch. The check is here because the five harnesses
 * are copies of each other, and whoever adds the first `<link>` should not have
 * to find this a second time. On a fully self-hosted app it costs a few
 ...
 * deliberately absent — a toast is the app confirming a write, and this take writes: the
 * `spending created` in the corner is part of what it shows.
 */
...
export class Demo {
   ...
   * the shared fixture. Its donut and its bars hit-test the cursor from ONE
   * handler on the graphic: an arc is a dash on a circle, a bucket a fraction of
   * a band, and neither has a box `moveTo` could aim at. The storyboard works
   * the point out from the geometry and sends the hand there.
   */
...
export const test = base.extend<{ demo: Demo }>({
  demo: async ({ page }, use) => {
    ...
    await page.close();
    if (video) await video.saveAs(join(OUT_DIR, "pfa-demo.webm"));
