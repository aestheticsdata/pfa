...
const useCharts = (periodType: string) => {
  ...
  const startDate =
    periodType === MONTHLY ? (from ? startOfMonth(from) : undefined) : from;
  const endDate =
    periodType === MONTHLY ? (from ? endOfMonth(from) : undefined) : to;

  const getCharts = async () => {
    const response = await privateRequest(
      `/spendings/charts?userID=${userID}&from=${startDate}&to=${endDate}`,
    );
